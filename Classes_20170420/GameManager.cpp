...
void makeData()
{
	...
	for (int i = 0; i < 11; i++)
	{
		...
		{
			nextupgrade[i] += nextupgrade[i] * bai[i];
		}
	}
	weapon = upgradelevel[0];
	level = upgradelevel[1];
	critical = upgradelevel[2] * 0.1f;
	fruitsTime = upgradelevel[3] * 0.5f;
	if (upgradelevel[4] == 0)
		autoTime = 0;
	else
		autoTime = 5.0f - 0.05f * (upgradelevel[4] - 1);
	if (upgradelevel[5] == 0)
		multiple = 0;
	else
		multiple = 5.0f - 0.05f * (upgradelevel[5] - 1);
	if (upgradelevel[6] == 0)
		wakeup = 0;
	else
		wakeup = 750 - 50 * upgradelevel[6];
	multiplepower = 0.02f * upgradelevel[7];
	wakeuppower = 0.02f * upgradelevel[8];
	dropBonus = 0.02f * upgradelevel[9];
