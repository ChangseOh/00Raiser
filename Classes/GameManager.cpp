...
int upgradelevel[11];
/*
장풍연마 : 새로운 장풍을 익힌다. 장풍당 나뭇잎 획득량↑
체력단련 : 더 강한 장풍을 발사한다. 위력+n%
크리티컬 : 10% 확률로 위력이 n% 더 강한 장풍 발사
요술열매 : 열매를 랜덤하게 뱉는다. 스톡된다. 사용하면 일정시간 위력 1.5배. 지속 시간 1초씩↑
기력충전 : 자동 발사. 자동 발사 간격이 빨라진다.
분신술 : 최초에는 분신이 오픈. 이후 분신의 위력↑ 1에서 오픈하고 11=110%가 맥시멈인거 보니 10%씩 오르나
각성 : n번 터치할 때마다 m초간 100% 확률로 크리 발동 (분신 제외) (유지시간은 각성강화로 늘어난다)
분신강화 : 분신의 장풍발사속도가 50% 빨라진다 : 최초 발사 간격에서 1/2씩 줄어든다고 보면 되나보다
각성강화 : 각성지속시간 1초 증가(최초 6초)
거름주기 : 나뭇잎을 n% 더 얻을 수 있다
약초꾼 : 오토퀘스트. n초가 지나면 나뭇잎을 수확해온다
*/
int upgrademax[11];//최대치
int upgraderate[11];//업그레이드시마다 적용되는 증액 배율
...
float multiplepower;//분신 파워
int wakeup;//각성. 0이면 각성 조건 없음. 이후는 각성에 필요한 연타 수
float wakeuppower;//각성파워
...
void makeData()
{
	for (int i = 0; i < 11; i++)
	{
		...
		{
			nextupgrade[i] += nextupgrade[i] * upgraderate[i];
		}
	}
	weapon = upgradelevel[0];//장풍연마
	level = upgradelevel[1];//체력단련
	critical = upgradelevel[2] * 0.1f;//크리티컬(위력)
	fruitsTime = 9 + upgradelevel[3];//열매 지속시간
	autoTime = 5.0f - 0.05f * (upgradelevel[4] - 1);//기력충전
	multiple = 5.0f - 0.05f * (upgradelevel[5] - 1);//분신의 위력
	wakeup = 750 - 50 * upgradelevel[6];//각성
	multiplepower = 0.02f * upgradelevel[7];//분신의 발사속도
	wakeuppower = 0.02f * upgradelevel[8];//각성지속시간 증가
	dropBonus = 0.02f * upgradelevel[9];//나뭇잎 획득량 증가
	autoquest = 40 + upgradelevel[10] * 10;//오토퀘스트획득량 업
}
